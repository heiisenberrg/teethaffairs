...
function DentalVisit(props) {
	...
	useEffect(() => {
		if (
			userList &&
			userList.length > 0 &&
			userList.findIndex(user => user.id === 'all-users') > -1
		) {
			userList.push({
				id: 'all-users',
				name: 'All Users',
	...
	const deleteDentalVisitById = () => {
		deleteDentalVisit(currentVisit.id);
		setShowModal(false);
		navigation.goBack();
		setRefresh(!refresh);
	};
				...
				<View style={ styles.content }>
					...
					<View style={ styles.avatarContent }>
						<Image
							style={ styles.avatar }
							source={
								visit && visit.profile_pic !== null
									? { uri: visit.profile_pic }
						...
						<Text style={ styles.normalText }>
							{moment(visit.issue_start_date).format('DD MMM YYYY')}
						</Text>
					</View>
				</View>
			</View>
	...
	return showEditCard ? (
				...
				<View row style={ styles.visitHeader }>
					...
					<View column style={ styles.titleWrapper }>
						...
						<View row style={ styles.nameWrapper }>
							...
							<Text style={ styles.light }>
								<Icon type={ 'SimpleLineIcon' } name={ 'clock' } color="#8E8B8B" />{' '}
								{moment(
									currentVisit.updated_on
										? currentVisit.updated_on
										: currentVisit.created_on
								).format('DD MMM YYYY')}
							</Text>
						</View>
					</View>
					<View column>
						<TouchableOpacity
							...
							<Icon type={ 'FontAwesome' } name={ 'edit' } size={ 14 } />
						</TouchableOpacity>
						<TouchableOpacity
							onPress={ () => setShowModal(true) }
							activeOpacity={ 0.8 }
							style={ styles.deleteWrapper }>
							<Icon type={ 'MaterialCommunityIcons' } name={ 'delete' } size={ 14 } />
						</TouchableOpacity>
					</View>
				</View>
		...
		<View>
			...
			<View center style={ styles.filterContainer }>
				<TouchableOpacity
					onPress={ () =>
						(userLists && userLists.length > 0 ? setVisible(!isVisible) : null)
					}
					style={ styles.filter }
					activeOpacity={ 0.95 }>
